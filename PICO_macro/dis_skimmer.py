...
time_current = array('f', [0]*(2*n_meas))
time_voltage = array('f', [0]*((2*n_meas/100)+1))
...
skimtree.Branch("time_current", time_current, "time_current[1900]/F")
skimtree.Branch("time_voltage", time_voltage, "time_voltage[20]/F")
...
for i in range(1, entries):
    ...
    if(trdrift[0] + trg1t[0] + trg1b[0] + trg2t[0] + trg2b[0] + trg3t[0] + trg3b[0] > 0 and tree.V_drift< tres_voltDR):
        ...
        for j in range(i, i+ m_peak_check):
            ...
            if(tree.I_G3B<-10000):
                ig3b_.append(tree.I_G3B+160600)
            else:
        ...
        if((last_event_saved == 0 or i-last_event_saved>delay) and to_write):
            ...
            for j in range(start, stop):
                ...
                if (tree.I_G3B<-2000):
                    ig3b[l] = copy.deepcopy(tree.I_G3B+160600)
                else:
                    ig3b[l] = copy.deepcopy(tree.I_G3B)
                idrift[l] = copy.deepcopy(tree.I_drift)
                time_current[l] = copy.deepcopy(j*acquisition_time)
                ...
                if (tree.V_G3T<10000 and tree.V_G1T!= prev_vg1t and m<20):
                    ...
                    time_voltage[m] = copy.deepcopy(j*acquisition_time)
                    m += 1
            ...
            integral_idrift[0] = integral(idrift, time_current, i)
            integral_ig1t[0] = integral(ig1t, time_current, i)
            integral_ig1b[0] = integral(ig1b, time_current, i)
            integral_ig2t[0] = integral(ig2t, time_current, i)
            integral_ig2b[0] = integral(ig2b, time_current, i)
            integral_ig3t[0] = integral(ig3t, time_current, i)
            integral_ig3b[0] = integral(ig3b, time_current, i)
            if i<950: n_ = i
            else: n_=950
